...
public class ClassEraserItem extends Item {
    public ClassEraserItem(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pHand) {
        ...
        pPlayer.getCapability(PlayerThirstProvider.PLAYER_THIRST).ifPresent(thirst -> {
            if(thirst.getpIndex() == 0){
                pPlayer.sendSystemMessage(Component.literal("Player is already classless"));
            }else {
                thirst.setPClass(0);
            }
        });
